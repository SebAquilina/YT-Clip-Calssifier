#!/bin/bash
cd "/home/user/YT-Clip-Calssifier/video8_veo/Project files"
...
while true; do
  ...
  REM=$(python3 -c "import json;s=json.load(open('state.json'));M=json.load(open('manifest.json'));print(sum(1 for b in M['beats'] for i in range(s['beats'].get(b['id'],{}).get('n_clips',1)) if s['beats'].get(b['id'],{}).get('jobs',{}).get(str(i),{}).get('status')!='downloaded'))" 2>/dev/null)
  ...
  echo "[finish] resuming gen ($REM left)" >> finish.log
  python3 generate.py >> gen.log 2>&1
done
echo "[finish] assembling" >> finish.log
python3 assemble.py >> asm.log 2>&1
echo "[finish] VIDEO8 ASSEMBLED" >> finish.log
