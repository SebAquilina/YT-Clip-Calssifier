...
for b in M["beats"]:
    ...
    if vm=="talking_head":
        cf=st["clip"]
        run([FF,"-y","-i",cf,"-vf",f"{DELOGO},scale=1280:720,fps=24",*ENC,seg])
    elif vm=="image_split":
        ...
        imfc=f"[1:v]scale={wIM*2}:-1,zoompan=z='min(zoom+0.0005,1.10)':d={int(D*24)}:s={wIM}x720:fps=24,setsar=1[R]"
        order="[L][R]" if thside=="left" else "[R][L]"
        run([FF,"-y","-i",cf,"-loop","1","-i",img,"-filter_complex",
             f"{thfc};{imfc};{order}hstack=2,format=yuv420p[v]","-map","[v]","-map","0:a","-t",f"{D:.3f}",*ENC,seg])
    elif vm=="image_full":
        ...
        run([FF,"-y","-loop","1","-i",img,"-i",aud,"-filter_complex",
             ...
             f"[1:a]aresample=48000,apad,atrim=0:{D:.3f},asetpts=PTS-STARTPTS[a]","-map","[v]","-map","[a]","-t",f"{D:.3f}",*ENC,seg])
    elif vm=="image_live":
