...
XFADE=float(os.environ.get("XFADE","0"))   # seconds; >0 = crossfade transitions between segments
if XFADE>0 and len(segments)>1:
    # Smoother transitions: chain xfade (video) + acrossfade (audio) across all segments.
    ...
    for k in range(1,len(segments)):
        off=acc-XFADE
        nv=f"[v{k}]"; na=f"[a{k}]"
        fc.append(f"{vlab}[{k}:v]xfade=transition=fade:duration={XFADE}:offset={off:.3f}{nv}")
        fc.append(f"{alab}[{k}:a]acrossfade=d={XFADE}:c1=tri:c2=tri{na}")
        vlab,alab=nv,na; acc=acc+durs[k]-XFADE
    fcs=";".join(fc)
    ...
    if not ok:
        print("xfade failed -> falling back to hard concat",flush=True); XFADE=0
if XFADE<=0 or not os.path.exists(out):
...
print("FINAL:",out,f"v={dur(out):.1f}s a={va}s xfade={XFADE}")
