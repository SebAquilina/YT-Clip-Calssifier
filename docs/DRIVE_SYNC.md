...
| `yt-clip-shared/records/` (one record per video) | `1_gB7b1SI1ZBnkj_1_X93FyapH2N0FaND` |
| `yt-clip-shared/by_label/` (one CSV per label) | `1o801LiZvJljq4APYfhu7y4S92DNc6LeL` |

`records/` is the source of truth (per-video). `by_label/` is the derived,
"divided by label" view — one `<label>.csv.gz` per action label (gzip of the
CSV, same reliability/CRC reasoning as records), regenerated by
`ytclip db-rebuild`. `index.json` at the root is the store snapshot.
