...
V=M["narrator_voice"]
def tts(text,dest):
    st,j=req("POST","/tts/generate",{"text":text,"voiceProvider":V["provider"],"voiceId":V["voiceId"],"modelId":V["modelId"]})
    jid=j.get("id")
...
def submit_video(prompt,muted,mode):
    body={"prompt":prompt,"model":M["video_model"],"aspectRatio":M["aspect"],
          "imageUrls":[REF],"videoInputMode":mode}
...
while (qi<len(tasks) or inflight) and time.time()<deadline:
    while len(inflight)<MAX_INFLIGHT and qi<len(tasks):
        ...
        mode="keyframes" if b["type"]=="character" else "ingredients"
        prompt=b["prompt"]
