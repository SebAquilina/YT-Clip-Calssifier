...
- **`outputs/shared_db/`** — a **concurrency-safe, cross-chat clip store** that many Claude sessions can read and extend at once (shards by `(label, video_id)` so writers never collide; talking-head / on-screen-text windows are flagged out of the usable-footage set). Backs the `YTA-video-maker` workflow. See [`docs/SHARED_DB.md`](docs/SHARED_DB.md); manage with `ytclip db-ingest`, `db-rebuild`, `db-stats`.
