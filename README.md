...
- **`outputs/shared_db/`** — a **concurrency-safe, cross-chat clip store** that many Claude sessions can read and extend at once, synced via **Google Drive** (one self-contained `records/<video_id>.json` per video, so writers never collide; talking-head / on-screen-text windows are flagged out of the usable-footage set; a rebuildable `by_label/` view keeps it "divided by label"). Backs the `YTA-video-maker` workflow. See [`docs/SHARED_DB.md`](docs/SHARED_DB.md) and [`docs/DRIVE_SYNC.md`](docs/DRIVE_SYNC.md); manage with `ytclip db-ingest`, `db-rebuild`, `db-coverage`, `db-todo`, `shotlist`, `drive-status`.
